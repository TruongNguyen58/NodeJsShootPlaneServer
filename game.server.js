...
game_server.onUpdateScore = function(obj) {
	log("xxxx");
	var _id = obj.gameId;
	var score = obj.score;
	var playerId = obj.playerId;
	var sender = obj.sender;
	log("gameId: " + _id + ", player: " +sender + ", score: " + score);
	if (games.hasOwnProperty(_id)) {
		games[_id].scores[sender] = score;
		if(score == games[_id].target) {
			endgame(_id);
		}
		else {
			var dataToSend = {};
